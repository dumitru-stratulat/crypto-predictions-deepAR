...
(function (PutDemo) {
    ...
    async function getNews(currencySymbol, currencyName) {
        ...
        let articles = (await axios.get("https://newsapi.org/v2/everything?q=" + currencyName + "&from=2023-03-01&to=2023-03-29&sortBy=popularity&language=en&apiKey=c9f743f4127e45c78d3993be2d26592b")).data;
        let counter = 1;
        ...
        for (const article of articles.articles.slice(0, 10)) {
            ...
            if (counter % 5 == 0 && counter != 0) {
                // insert in dynamo db
                documentClient.batchWriteItem(params, (result, err) => {
                    if (err)
                        console.log(err);
                    else
                        console.log(result);
